...
const QUICK_QUESTIONS_ES = [
  '¿Qué es Fallas?',
  '¿Qué pasa hoy?',
  '¿Mejores sitios para la Cremà?',
  '¿Cómo me muevo por Valencia?',
  'Consejos para mascotas',
  '¿Qué es una mascletà?',
];

export const FallasChatbot: React.FC = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error ?? 'Could not connect to the assistant.');
        setMessages((prev) => [
          ...prev,
          { role: 'model', text: "I couldn't respond right now. Please try again." },
        ]);
        return;
      }
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'model', text: 'Something went wrong. Check your connection and try again.' },
      ]);
    } finally {
      setIsLoading(false);
          ...
          {messages.length === 0 ? (
            <div className="max-w-lg mx-auto text-center">
              ...
              <p className="text-terreta-dark/70 text-sm mb-5">
                {language === 'es'
                  ? 'Pregunta lo que quieras sobre Fallas: calendario, consejos, dónde ir, seguridad, glosario...'
                  : 'Ask me anything about Fallas: schedule, tips, where to go, safety, glossary...'}
              </p>
              ...
              {isLoading && (
                <div className="flex gap-2.5">
                  ...
                  <div className="bg-terreta-card border border-terreta-border rounded-2xl px-3.5 py-2.5 flex items-center gap-2">
                    ...
                    <span className="text-terreta-dark text-sm">
        ...
        <form onSubmit={handleSubmit} className="flex-shrink-0 p-3 border-t border-terreta-border bg-terreta-card/50">
          <div className="flex gap-2 items-end border border-terreta-border rounded-xl bg-terreta-bg focus-within:ring-2 focus-within:ring-terreta-accent/50 focus-within:border-terreta-accent transition-all">
            <textarea
              ...
              placeholder={language === 'es' ? 'Pregunta sobre Fallas…' : 'Ask about Fallas…'}
              rows={1}
              ...
              disabled={isLoading}
              aria-label={language === 'es' ? 'Mensaje al asistente de Fallas' : 'Message to Fallas assistant'}
            />
            <button
              type="submit"
